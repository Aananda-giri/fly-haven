...
def main():
    ...
    n=soil.node_tree.nodes;l=soil.node_tree.links
    patch=n.new('ShaderNodeTexNoise');patch.inputs['Scale'].default_value=23;patch.inputs['Detail'].default_value=3
    coord=n.new('ShaderNodeNewGeometry');l.new(coord.outputs['Position'],patch.inputs['Vector'])
    ramp=n.new('ShaderNodeValToRGB')
    ramp.color_ramp.elements.remove(ramp.color_ramp.elements[1])
    colors=[(.25,(.028,.016,.008,1)),(.47,(.11,.06,.028,1)),(.56,(.065,.08,.015,1)),(.67,(.11,.16,.026,1)),(.82,(.035,.07,.012,1))]
    ramp.color_ramp.elements[0].position=colors[0][0];ramp.color_ramp.elements[0].color=colors[0][1]
    for pos,col in colors[1:]:ramp.color_ramp.elements.new(pos).color=col
    l.new(patch.outputs['Fac'],ramp.inputs[0]);l.new(ramp.outputs[0],n.get('Principled BSDF').inputs['Base Color'])
    import numpy as np
    ...
    fx,fy=-.18,.11
    fruit=ellipsoid('Split fallen fruit',(fx,fy,terrain_height(fx,fy)-.005),(.052,.046,.006),pulp,4)
    for v in fruit.data.vertices:
        a=math.atan2(v.co.y,v.co.x);factor=1+.035*math.sin(7*a)+.02*math.sin(13*a)
        v.co.x*=factor;v.co.y*=factor
    seed=material('Fruit seeds',(.21,.065,.011),.45,1400)
    for i in range(85):
        a=RNG.random()*6.28;r=math.sqrt(RNG.random())*.92
        x=fx+.052*r*math.cos(a);y=fy+.046*r*math.sin(a);z=terrain_height(fx,fy)-.005+.006*math.sqrt(1-r*r)
        ellipsoid('Pulp vesicle',(x,y,z),(.0007,.0013,.00035),seed if i%5==0 else pulp,1)
    for i in range(13):
    ...
    bpy.ops.object.light_add(type='AREA',location=(0,-.3,.7));fill=bpy.context.object;fill.data.energy=22;fill.data.shape='DISK';fill.data.size=1.2;fill.data.color=(.65,.8,1)
    ...
    s=bpy.context.scene;s.render.fps=24;s.unit_settings.system='METRIC';s.view_settings.view_transform='AgX';s.view_settings.exposure=.5
