...
from heaven.blender.motion import Timeline, Blend, Travel, FPS, OFFSET, ease, angle_delta, terrain_height
...
def append_fly(suffix,scale=1.):
    with bpy.data.libraries.load(str(RIG_BLEND),link=False) as (src,dst):dst.objects=list(src.objects)
...
def positions(tl,end):
    ...
    for frame in range(round(end*FPS)+1):
        ...
        if present:
            ...
            if court:last_fh+=angle_delta(last_fh,h)*(1-math.exp(-1/FPS/.3))
        fh.append(last_fh)
        female.append([*fxy,terrain_height(*fxy)])
        male.append([*xy,terrain_height(*xy)+float(r['fly_altitude'])])
...
def bake_fly(root,rig,objects,feet,rows,path,headings,is_female=False):
    ...
    for frame,(r,xyz,h) in enumerate(zip(rows,path,headings)):
        ...
        for foot in feet:
            rest=root.matrix_world @ foot['rest'];rest.z=terrain_height(rest.x,rest.y)+foot['clearance']
            ...
            if pose=='groom' and foot['part']=='Front':
                special=root.matrix_world @ Vector((.0015*(1 if foot['side']=='L' else -1),-.0065,.004))
                ...
                special=root.matrix_world @ foot['rest'];special.z+=.002
            elif pose in ('mate','mount_approach'):
                special=root.matrix_world @ foot['rest'];special.z+=.001 if foot['part']=='Front' else 0
...
def camera(rows,path,female,headings):
    ...
    data.lens=48;data.clip_start=.0005;data.clip_end=30;data.dof.use_dof=True;data.dof.aperture_fstop=7.1
    ...
    for frame,(r,xyz,h) in enumerate(zip(rows,path,headings)):
        ...
        center=xyz+np.array([0,0,.0045]);court=r['state']=='COURT' and r['female_present']
        if court:center=(center+female[frame]+np.array([0,0,.005]))*.5
        desired=center-f*(.067 if court else .057)+side*.047+np.array([0,0,.031])
        if pos is None:pos=desired.copy();target=center.copy()
        pos+=(desired-pos)*(1-math.exp(-1/FPS/.3));target+=(center-target)*(1-math.exp(-1/FPS/.16))
