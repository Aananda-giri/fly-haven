...
def surface_height(x,y):
    """Walkable ground plus the shallow fruit and resting stone caps."""
    z=terrain_height(x,y)
    for cx,cy,rx,ry,base,height in ((-.18,.11,.052,.046,-.005,.006),(-.30,.15,.032,.025,-.003,.0035)):
        q=((x-cx)/rx)**2+((y-cy)/ry)**2
        if q<1:z=max(z,terrain_height(cx,cy)+base+height*math.sqrt(1-q))
    return z
